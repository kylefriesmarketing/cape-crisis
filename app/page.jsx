...
import { World, HEROES, CHAPTERS, WEAPONS } from '@/game/engine.mjs';
...
const snapshot=g=>({...g.summary(),players:g.players.map(p=>({...p})),boss:g.enemies.find(e=>e.boss),combo:g.combo,upgrades:g.upgrades});
export default function Home(){
 ...
 function pause(){if(settings||help)return;const rt=runtime.current,g=rt?.world;if(!g)return;if(g.status==='playing')g.status='paused';else if(g.status==='paused')g.status='playing';else return;rt.controls.clear();sync(g);}
 function start(){
  ...
  rt.world=new World({hero,difficulty,coop});rt.controls.clear();rt.accumulator=0;rt.audio.step=0;rt.audio.next=rt.audio.ctx?rt.audio.ctx.currentTime+.04:0;
  setNotice('');setSettings(false);setHelp(false);sync(rt.world);
 }
 ...
 actions.current={pause,start,home,upgrade};
 ...
 useEffect(()=>{
  ...
  const rt={renderer,audio,controls,world:null,accumulator:0};runtime.current=rt;
  let raf=0,last=performance.now(),lastHud=0,alive=true;
  function frame(now){
   if(!alive)return;const dt=Math.min(.12,(now-last)/1000);last=now;const g=rt.world;audio.music=prefsRef.current.music;audio.effects=prefsRef.current.effects;
   if(g){
    audio.setState(g.status==='playing',g.chapter);
    if(g.status==='playing'){rt.accumulator+=dt;let steps=0;while(rt.accumulator>=1/60&&steps++<8&&g.status==='playing'){g.step(1/60,controls.read(g,prefsRef.current.autoAim));rt.accumulator-=1/60;}}else {rt.accumulator=0;if(g.status==='paused')controls.read(g,prefsRef.current.autoAim);}
    for(const e of g.drainEvents())audio.event(e);
    ...
    renderer.render(g,{shake:prefsRef.current.shake,touch:controls.usingTouch?controls.touch:null,aim:controls.mouse.seen&&!controls.usingTouch&&!prefsRef.current.autoAim?controls.mouse:null});
 ...
 return <main className={playing?'cabinet in-game':'cabinet'}>
  ...
  {!playing&&<div className="title-screen">
   ...
   <div className="launch-deck">
    ...
    <footer className="menu-footer"><Button variant="ghost" onClick={()=>setHelp(true)} className="text-button"><Crosshair size={15}/> HOW TO PLAY</Button><span><AudioLines size={15}/> SIX CHAPTERS. TURN IT UP.</span><span className="credit">ONE PLANET. NO PRESSURE.</span></footer>
   </div>
  </div>}
  {playing&&hud&&<>
   <header className="game-hud"><div className="health-panels">{hud.players.map(p=><div className="health-panel" key={p.id} style={{'--hero':p.def.color}}><span className="hud-hero">{p.id?'P2 / ':''}{p.def.name}<small>{p.hp<=0?'DOWN':Math.ceil(p.hp)+' / '+p.maxHp}</small></span><div className="health-track"><i style={{width:Math.max(0,p.hp/p.maxHp)*100+'%'}}/></div></div>)}</div>
    <div className="score-panel"><span>SCORE</span><strong>{num(hud.score).padStart(6,'0')}</strong>{hud.combo>=12&&<em>×{1+Math.min(9,Math.floor(hud.combo/12))} / {hud.combo} CHAIN</em>}</div><div className="run-time"><span>ISSUE {String(hud.chapter).padStart(2,'0')}</span><strong>{clock(hud.time)}</strong></div><Button variant="ghost" className="icon-button pause-button" onClick={pause} aria-label={view==='paused'?'Resume game':'Pause game'}>{view==='paused'?<Play/>:<Pause/>}</Button>
   </header>
   {hud.boss&&<div className="boss-hud"><span>{hud.boss.type==='mother'?'THE MOTHERSHIP':'THE COLLECTION AGENT'}</span><div><i style={{width:Math.max(0,hud.boss.hp/hud.boss.maxHp)*100+'%'}}/></div></div>}
   <div className="combat-footer"><div className="power-panels">{hud.players.map(p=><div className="power-panel" key={p.id} style={{'--hero':p.def.color}}>
    ...
    <div className="weapon-line">{p.weapon?<><span style={{color:WEAPONS[p.weapon].color}}>{WEAPONS[p.weapon].name}</span><b>{Math.ceil(p.weaponTime)}s</b></>:<span>{p.hero==='solar'?'PHOTON BOLTS':p.hero==='volt'?'ARC LIGHTNING':'GRAVITY PUNCH'}</span>}<small>{p.dashCooldown>0?'DASH '+p.dashCooldown.toFixed(1)+'s':'DASH READY'}</small></div>
  ...
  <Dialog open={view==='upgrade'} onOpenChange={()=>{}}><DialogContent className="game-dialog upgrade-dialog" showCloseButton={false}><span className="dialog-kicker">NEXT ISSUE / POWER UP</span><DialogTitle className="dialog-heading">THICKER PLOT.<br/><em>BIGGER POWERS.</em></DialogTitle><DialogDescription className="dialog-copy">Choose one permanent upgrade.{coop?' Both heroes get it.':' It lasts for the rest of this run.'}</DialogDescription><div className="upgrade-grid">{hud?.upgrades.map(u=>{const Icon=icons[u.icon];return <Button key={u.id} variant="outline" className="upgrade-card" onClick={()=>upgrade(u.id)}><Icon/><strong>{u.name}</strong><p>{u.desc}</p><span>TAKE IT <ArrowRight size={16}/></span></Button>;})}</div></DialogContent></Dialog>
  <Dialog open={view==='paused'&&!settings&&!help} onOpenChange={open=>{if(!open)pause();}}><DialogContent className="game-dialog pause-dialog" showCloseButton={false}><span className="dialog-kicker">EVEN HEROES NEED A MINUTE.</span><DialogTitle className="dialog-heading">HOLD THAT<br/><em>THOUGHT.</em></DialogTitle><DialogDescription className="dialog-copy">The invasion can wait.</DialogDescription><Button className="launch-button" onClick={pause}><Play/> BACK TO THE FIGHT <ArrowRight/></Button><div className="pause-options"><Button variant="outline" onClick={()=>setSettings(true)}><Settings2/> Settings</Button><Button variant="outline" onClick={()=>setHelp(true)}><Crosshair/> Controls</Button><Button variant="outline" onClick={fullscreen}><Expand/> Fullscreen</Button></div><Button variant="ghost" className="quit-button" onClick={home}>END RUN & RETURN TO TITLE</Button></DialogContent></Dialog>
  <Dialog open={ended} onOpenChange={()=>{}}><DialogContent className="game-dialog results-dialog" showCloseButton={false}><span className="dialog-kicker">{view==='victory'?'NOVA CITY OWES YOU ONE.':'HEROISM IS A WORK IN PROGRESS.'}</span><DialogTitle className="dialog-heading">{view==='victory'?<>PLANET<br/><em>SAVED.</em></>:<>TO BE<br/><em>CONTINUED…</em></>}</DialogTitle><DialogDescription className="dialog-copy">{view==='victory'?'The mothership is scrap. The coffee is still warm.':'One more run. This time, make them pay for parking.'}</DialogDescription><div className="result-score"><span>FINAL SCORE</span><strong>{num(hud?.score)}</strong>{hud?.score>=best&&hud?.score>0&&<em><Trophy size={16}/> PERSONAL BEST</em>}</div><div className="result-stats"><div><strong>{clock(hud?.time||0)}</strong><span>SURVIVED</span></div><div><strong>{num(hud?.kills)}</strong><span>ALIENS EVICTED</span></div><div><strong>{hud?.bestCombo||0}</strong><span>BEST CHAIN</span></div></div><Button className="launch-button" onClick={start}>ONE MORE ISSUE <ArrowRight/></Button><Button variant="ghost" className="quit-button" onClick={home}>CHANGE HERO / TITLE SCREEN</Button></DialogContent></Dialog>
  <Dialog open={settings} onOpenChange={setSettings}><DialogContent className="game-dialog settings-dialog"><span className="dialog-kicker">TUNE YOUR FREQUENCY</span><DialogTitle className="dialog-heading">THE<br/><em>CONTROLS.</em></DialogTitle><DialogDescription className="dialog-copy">Saved on this device. Sound starts when you play.</DialogDescription>{[['music','The soundtrack','Original reactive synth-rock. Headphones encouraged.',Music2],['effects','Combat sounds','Every zap, dash, crunch, and glorious explosion.',Volume2],['shake','Screen shake','A little extra impact. Switch off for a calmer screen.',Sparkles],['autoAim','Auto aim & fire','Focus on movement. Your hero targets the nearest alien.',Crosshair]].map(([key,title,desc,Icon])=><label className="setting-row" key={key}><Icon size={22}/><span><strong>{title}</strong><small>{desc}</small></span><Switch checked={prefs[key]} onCheckedChange={v=>setting(key,v)} aria-label={title}/></label>)}<Button variant="outline" className="wide-button" onClick={fullscreen}><Expand/> TOGGLE FULLSCREEN</Button></DialogContent></Dialog>
  <Dialog open={help} onOpenChange={setHelp}><DialogContent className="game-dialog help-dialog"><span className="dialog-kicker">THE VERY SHORT TRAINING MONTAGE</span><DialogTitle className="dialog-heading">LOOK ALIVE,<br/><em>HERO.</em></DialogTitle><DialogDescription className="dialog-copy">Survive five escalating minutes, then destroy the mothership. Keep kills close together to build a score multiplier.</DialogDescription><div className="controls-list">{[['MOVE','W A S D'],['AIM & FIRE','Mouse + hold click / Arrow keys'],['DASH THROUGH DANGER','Space / Shift'],['UNLEASH SUPER','E / Q · needs 100% charge'],['PAUSE','Esc / P']].map(([a,b])=><div key={a}><span>{a}</span><b>{b}</b></div>)}</div><p className="help-note"><Gamepad2/> Controller: left stick moves, right stick fires, A / LB dashes, X / RB unleashes super, Menu pauses.</p><p className="help-note"><Crosshair/> Touch: drag the left half to move and the right half to aim and fire. Use DASH and SUPER. Landscape gives you more room.</p><p className="help-note"><Heart/> Co-op: Player 1 uses keyboard and mouse; Player 2 uses the first controller. Stand near a downed teammate for 2.5 seconds to revive them.</p><p className="help-note"><Shield/> Grab pickups for health, shields, super energy, or a 16-second weapon. Glowing circles warn you where danger will land.</p></DialogContent></Dialog>
