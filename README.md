...
## Validation
16 automated checks pass, including three 330-second combat soaks with damage suppressed to reach every chapter. These check runtime state and mechanics, not human difficulty balance. Renderer calls are validated with a canvas mock; input mappings use simulated keyboard, pointer and gamepad events. Physical controllers, device touch and visual browser playtesting have not been performed.
